...
const RumCard = memo(({ 
  ...
  return (
    <div
      className={`absolute inset-0 w-full sm:w-80 md:w-96 h-[400px] sm:h-[450px] md:h-[500px] ${rum.cardColor} ${index === currentIndex ? 'active-card' : ''} rounded-2xl p-4 sm:p-6 md:p-8 text-center transition-all duration-700 ease-in-out cursor-pointer`}
      style={{
    ...
    >
      ...
      <div className={`absolute top-2 left-2 w-8 sm:w-10 md:w-12 h-8 sm:h-10 md:h-12 border-t-2 border-l-2 ${borderColor} rounded-tl-xl z-20`}></div>
      <div className={`absolute top-2 right-2 w-8 sm:w-10 md:w-12 h-8 sm:h-10 md:h-12 border-t-2 border-r-2 ${borderColor} rounded-tr-xl z-20`}></div>
      <div className={`absolute bottom-2 left-2 w-8 sm:w-10 md:w-12 h-8 sm:h-10 md:h-12 border-b-2 border-l-2 ${borderColor} rounded-bl-xl z-20`}></div>
      <div className={`absolute bottom-2 right-2 w-8 sm:w-10 md:w-12 h-8 sm:h-10 md:h-12 border-b-2 border-r-2 ${borderColor} rounded-br-xl z-20`}></div>

      
      {/* Image */}
      <div className="relative mb-4 sm:mb-6 overflow-hidden rounded-xl h-36 sm:h-40 md:h-48 z-20">
        <img 
          src={rum.image} 
          alt={rum.name}
      ...
      <div className="relative z-20">
        <h3 className={`text-xl sm:text-2xl font-bold mb-2 sm:mb-3 ${rum.textColor} font-eb-garamond`}>
          {rum.name}
        </h3>
        
        <p className={`${rum.name === "Chocolate Flavored" ? 'text-[#FFF8E1]' : `${rum.textColor}/80`} mb-4 sm:mb-6 leading-relaxed text-sm sm:text-base font-medium text-left`}>
          {rum.description}
        </p>
...
const SectionHeader = memo(() => {
  return (
    <div className="text-center mb-12 sm:mb-16 md:mb-20 animate-fade-in-up">
      <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-white mb-4 sm:mb-6 md:mb-8 font-eb-garamond">
...
const NavigationButtons = memo(({ onPrev, onNext }: NavigationButtonsProps) => {
  return (
    <div className="absolute -bottom-12 left-0 right-0 flex justify-center items-center space-x-8 sm:hidden pb-4">
      <button
        onClick={onPrev}
        className="bg-rum-gold/20 border border-rum-gold text-rum-gold hover:bg-rum-gold hover:text-rum-black backdrop-blur-sm p-3 rounded-full transition-all duration-300 z-40"
        aria-label="Previous rum"
      >
...
const Collection = () => {
  ...
  const customStyles = `
    ...
    .active-card:hover {
      transform: translateX(0) translateY(0px) scale(1) rotate(0deg) !important;
      box-shadow: 0 30px 60px -15px rgba(0, 0, 0, 0.6) !important;
    }
    
    @media (max-width: 640px) {
      .active-card:hover {
        transform: translateX(0) translateY(-5px) scale(1) rotate(0deg) !important;
        box-shadow: 0 20px 40px -10px rgba(0, 0, 0, 0.6) !important;
      }
      
      .carousel-paused .active-card {
        box-shadow: 0 15px 30px -8px rgba(0, 0, 0, 0.6) !important;
      }
    }
  `;
  ...
  const getCardPosition = useCallback((index: number) => {
    ...
    switch (position) {
      ...
      case 1: // Next card (right)
        return {
          transform: `translateX(${isMobile ? '60px' : '100px'}) translateY(0) scale(${isMobile ? '0.8' : '0.9'}) rotate(${isMobile ? '3deg' : '5deg'})`,
          zIndex: 20,
          opacity: isMobile ? 0.9 : 0.95,
          boxShadow: isMobile ? '10px 10px 20px -5px rgba(0, 0, 0, 0.5)' : '15px 15px 30px -5px rgba(0, 0, 0, 0.5)'
        };
  ...
  }, [currentIndex, rums.length, windowWidth]);
  ...
  return (
    <section id="collection" className="py-24 bg-gradient-to-br from-rum-black via-rum-red-dark to-rum-black relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="relative h-[520px] sm:h-[550px] md:h-[600px] flex items-center justify-center">
          {/* Cards Container */}
          <div 
            className={`relative w-[280px] sm:w-80 md:w-96 h-[400px] sm:h-[450px] md:h-[500px] mx-auto ${isPaused ? 'carousel-paused' : ''}`}
        ...
        <div className="mt-16 sm:mt-20 md:mt-24 relative rounded-2xl overflow-hidden animate-scale-in group">
          <img 
            src="/Images/all-rums.jpg"
            alt="Master's Distillery Collection"
            className="w-full h-[28rem] sm:h-80 md:h-96 object-cover group-hover:scale-105 transition-transform duration-700"
          />
          <div className="absolute inset-0 bg-gradient-to-r from-rum-black/80 via-rum-red/40 to-transparent flex items-center">
            <div className="p-4 sm:p-6 md:p-8 text-white max-w-xs sm:max-w-sm md:max-w-2xl backdrop-blur-sm bg-rum-black/20 rounded-2xl m-4 sm:m-6 md:m-8 border border-rum-gold/40 hover:border-rum-gold/50 transition-all duration-300">
              <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-3 sm:mb-4 md:mb-6 bg-gradient-to-r from-white to-rum-gold bg-clip-text text-transparent font-eb-garamond">Not Your Grandpa's Candy
              </h3>
              <p className="text-base sm:text-lg text-white/90 leading-relaxed mb-4 sm:mb-6 md:mb-8">
